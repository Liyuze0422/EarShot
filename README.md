...
- ⚠️ **话题续接仍有弱点。** 已经做了话题粘滞（`detect_topic_sticky` + STICKY_BOOST=3.0，实测 hit@1 69.2% → 84.6%）；但面试官用「那这个呢」这类**完全没有实词**的追问时，检索还是可能停在上一题上（实测 39 条真实提问里 6 条属于此类）。
...
- [ ] 追问续接还有余量：**完全无实词**的「那这个呢」仍可能停在上一题（已有话题粘滞兜底，见上面「诚实边界」）
