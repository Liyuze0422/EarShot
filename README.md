...
| [知识库整理清单](docs/知识库整理清单.md) | **材料怎么放、怎么命名、怎么切块** —— 直接决定检索命中率，配 `tools/kb_audit.py` 自检、`tools/kb_expand.py` 补"面试官换说法" |
...
- [ ] 追问续接还有余量：**话题被 HR 中断后切回项目**的追问（"还有哪些角色""你做的占比多少"）仍会停在 HR 材料上。
      0.9.3 把能想到的**六个词面改法逐个实测，全是否定的**（置信度、查询窗口、HR 词表、HR 加权强度、
      sticky 判定收紧 —— 详见 [性能实测](docs/性能实测.md) 的「检索的真实边界」）。
      这两句的字面词在 HR 材料里更密集，**要分开得靠语义**，词表/权重分不开；剩下一条是 ASR 听错。
      **不要指望用话题词表绕过去**：把「角色/占比」补进 `config/topic_terms.json` 实测 **零变化**；
      再补「架构/功能」这类泛词才把 hit@1 提到 92.3%（修的是另一条 #26），
      但泛词在真实面试里会到处误触发，收益与风险都没验证 —— 不建议这么改
